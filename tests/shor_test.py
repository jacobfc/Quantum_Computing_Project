...
def shor_quantum(N, m, n, i):
    ...
    for j in range(n):
        ...
        if (not found[value]):
            ...
            number += 1
            found[value] = True
    output_register = output_register / math.sqrt(number) #normalize the state
    ...
    number = 0 # number of remaining states (need this to renormalize the array)
    for j in range(n):
        value = m**j % N
        if (value == result):
    ...
    input_register = input_register / math.sqrt(number) #normalize the state
...
def contd_fraction(x):
    # Treshold value needed because of the imprecision of floats so that
...
which is given by s = y/n, where y is the measured outcome of the input register
and n is the size of the basis (input register).
